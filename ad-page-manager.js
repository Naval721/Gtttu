...
try {
     ...
     */
    window.closeAllAdWindows = function closeAllAdWindows() {
        let closedCount = 0;
     ...
     */
    window.clearBrowserHistory = function clearBrowserHistory() {
        try {
     ...
     */
    window.closeAllPages = async function closeAllPages() {
        console.log('🔥 CLOSING ALL PAGES...');

        let totalClosed = 0;

        // 1. Close tracked ad windows
        totalClosed += window.closeAllAdWindows();
    ...
    window.addEventListener('popstate', function (e) {
        try {
            ...
            console.warn('Popstate handler error:', err);
        }
    });
    ...
    console.error('❌ Ad page manager failed to load:', error);
    // Define fallback empty functions so the app still works
    window.closeAllAdWindows = function () { return 0; };
    window.closeAllPages = async function () { return 0; };
    window.clearBrowserHistory = function () { return false; };
}
